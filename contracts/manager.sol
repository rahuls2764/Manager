...
contract Manager is Ownable, ReentrancyGuard {
    ...
    function submitQuizResult(
        ...
        if (_score >= quizzes[_quizId].minScore) {
            ...
            string memory tokenURI = string(
                abi.encodePacked("ipfs://", _resultIPFSHash)
            );

            // Directly mint the NFT to the student's address
            uint256 tokenId = quizNFT.mintNFT(msg.sender, tokenURI); // Mint NFT to msg.sender
    ...
    function getStudentQuizResult(
        address _student,
        uint256 _quizId
    ) external view returns (bool completed, string memory resultIPFSHash) {
        return (
    ...
    function getStudentStats(
        address _student
    ) external view returns (uint256 totalScore, uint256 quizzesTaken) {
